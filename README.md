...
### agent-shell(Claude を Emacs から操作)のエンジン

`M-x agent-shell` で Claude 等の AI エージェントを Emacs バッファから対話操作する。モデル実行は外部 ACP アダプタ `claude-agent-acp` に委譲するので、これを 1 度入れる。

1. **ACP アダプタを導入**(本マシンは volta 管理。素の `npm install -g` は使わない):
...
2. **PATH に入ったか確認**:
   ...
   ```sh
   which claude-agent-acp     # ~/.volta/bin/claude-agent-acp が返れば OK
   ```

3. **Claude サブスク認証**: `init.el` は `:login t`(サブスクリプション認証)で設定済み。**Claude Code で一度ログイン済みであること**(未ログインなら `claude` CLI で一度ログインする)。`:login t` は保存済みトークン(macOS は Keychain)を再利用するため、`claude` バイナリが PATH に無くても動く見込み。万一 GUI Emacs から認証に失敗する場合のフォールバックは、`~/.claude/local` を `~/.zshenv` の PATH に追加する(`exec-path-from-shell` が拾う)か、OAuth トークン / API キーを **auth-source 経由**で渡す(`init.el` の `agent-shell` セクションにコメント例あり。鍵を直書きしない)。

4. **状態ディレクトリ `.agent-shell/` の除外**: agent-shell はセッション状態を作業ディレクトリ直下の `.agent-shell/` に書く。これはグローバル無視ファイル `~/.config/git/ignore` に `.agent-shell`(スラッシュ無し)を 1 行入れて除外する運用(本マシンは設定済み)。agent-shell 自身は追跡対象の `.gitignore` を書き換えず `.git/info/exclude` に追記しようとするが、グローバル ignore で既に「無視済み」と判定されていればそれも触らない。

5. (任意)画像の貼り付けを使う場合、macOS は `brew install pngpaste`。

`claude-agent-acp` が無いマシンでは `use-package :if` で `agent-shell` 関連が全体スキップされ、起動エラーにはならない(ripgrep / cmigemo と同じ扱い)。

### vterm(Emacs 内ターミナル)のエンジン

`M-x vterm` で起動する libvterm ベースのターミナル。**ネイティブモジュール**(`vterm-module`)を使うため、各マシンでビルドツールが要る。

1. **ビルドツールを導入**(全機 Apple Silicon = arm64、brew は `/opt/homebrew`):

   ```sh
   brew install cmake libtool   # cmake は必須、libtool は保険
   ```

   加えて C コンパイラ(macOS は Xcode Command Line Tools=`xcode-select --install`)が必要。

2. **ネイティブモジュールは git で同期しない**。`vterm-module.so` は **各 Mac で初回 `M-x vterm` 時にローカルビルド**される(tree-sitter の dylib と同じ思想で、`.gitignore` で除外=リポジトリには `.el` ソースのみ)。`cmake` が無いと初回ビルドが失敗するので、上記を先に入れておくこと。Apple Silicon でビルドされる `vterm-module.so` が arm64 であることは確認済み。

3. `module-file-suffix` が nil(モジュール非対応ビルド)のマシンでは `use-package :if` で `vterm` 関連が全体スキップされ、起動エラーにはならない(ripgrep / cmigemo / agent-shell と同じ扱い)。
...
### AI エージェント連携(agent-shell / Claude)

Claude 等の AI コーディングエージェントを Emacs バッファから対話操作する。モデル実行は外部 ACP アダプタ `claude-agent-acp` に委譲(導入は「新マシンセットアップ → agent-shell」参照)。起動はグローバルキーを作らず **`M-x agent-shell`**(他キーとの衝突回避)。認証はサブスクリプション(`:login t`、Keychain のトークン再利用。鍵は init.el に書かない)。
...
|---|---|
| `RET` | プロンプト送信 |
| `M-J` / `S-<return>` | 改行(送信せず複数行入力) |
| `C-c C-c` | 実行中断＋保留中の権限を全却下(`agent-shell-interrupt`) |
| `TAB` / `S-TAB` | 権限ボタン等の要素間移動 |
| `C-c C-v` | モデル設定 |
| `C-c C-m` | セッションモード設定 |
| `C-c C-t` | thought level(推論の深さ)設定 |
| `C-<tab>` | セッションモード循環 |
| `M-r` | 入力履歴検索 |
| `C-<up>` / `M-p` | 前の入力 |
| `C-<down>` / `M-n` | 次の入力 |
| `C-c C-o` | viewport ⇔ shell 切替 |
| `M-x agent-shell-view-traffic` | ACP 通信を監査表示 |
| `M-x agent-shell-open-transcript` | トランスクリプトを開く |

**権限フロー**: ファイル編集やコマンド実行は即時には走らず、バッファ内の権限ボタンで承認してから実行される(`TAB` で移動 → `RET` で承認)。実行を止めたいときは `C-c C-c` で中断＋保留中の権限を一括却下する。

### vterm(Emacs 内ターミナル)

libvterm ベースの本物のターミナルを Emacs バッファ内で動かす。起動は **`M-x vterm`**(グローバルキーは未割当)。シェルは `$SHELL`(本マシンは `/bin/zsh`)。導入は「新マシンセットアップ → vterm」参照。

採用理由: 当初候補の `eat` は [issue #262](https://codeberg.org/akib/emacs-eat/issues/262)(全角文字・罫線が連続すると O(n²) で再描画がハング、Claude Code の起動バナーで再現)があり不採用。`vterm` は C 実装の libvterm に描画を委ねるため同バナーも即描画される。

| キー / 操作 | 機能 |
|---|---|
| `M-x vterm` | 起動(新規ターミナルバッファ) |
| `C-c C-t` | コピーモードに入る(範囲選択 → `RET` で kill ring へ。別バッファに `C-y`) |
| `claude` | vterm 内で実行可(既存 alias。対話 zsh が解決) |

**ディレクトリ追跡(`cd` 後に `C-x C-f` が追従)には zsh 側設定が必要**。ディレクトリ追跡は zsh が OSC エスケープ列で「いまの作業ディレクトリ」を vterm に通知することで実現する。設定は別リポジトリ管理の zsh(ZDOTDIR=`~/.config/zsh`、powerlevel10k 構成)に置く。`~/.config/zsh/.zshrc` に次を追記する(p10k はプロンプトを管理するため `PROMPT` 直編集ではなく `precmd` フックから通知する):

```zsh
# vterm (Emacs) 連携: ディレクトリ追跡。vterm 内でのみ有効。
# powerlevel10k 構成のため PROMPT 直編集ではなく precmd で通知する。
if [[ "$INSIDE_EMACS" = vterm ]]; then
  ...
  vterm_set_directory() { vterm_printf "51;A$(whoami)@$(hostname -s):$(pwd)"; }
  add-zsh-hook precmd vterm_set_directory
fi
```

ウィンドウをリサイズしても既存行が rewrap されないのは vterm の既定仕様(欠陥ではない)。新しく描画される行から新しい幅が反映される。
