...
| `C-z c` / `C-z C-z` / `C-z 0` / `n` / `p` / `b` / `u` / `f` / `1`..`9` | tab-bar(new / recent / close / next / prev / バッファのタブへ・無ければ新タブ / undo / redo / 番号ジャンプ) | 組み込み |
...
| `C-z b` | `my-tab-find-buffer`(指定バッファを表示中のタブへ移動 / どのタブにも無ければ新タブで開く。旧 elscreen の `C-z b` = `elscreen-find-and-goto-by-buffer` 相当) |
