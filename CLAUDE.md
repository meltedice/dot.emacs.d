...
- [x] 組み込み UX 定番(セッション永続化・操作補助) — **新規追加**(旧設定では未使用、「定番なのに未導入」だったものをまとめて有効化、ユーザー判断)。すべて Emacs 同梱・外部パッケージ非依存。`init.el`「組み込み UX」セクション参照。**採用**: ① `recentf-mode`(最近ファイル、`C-x C-r`→`recentf-open`(Emacs 29+ 補完版)に bind。既定 `find-file-read-only` を上書き=読取専用 visit は view-read-only 側で代替済み。`recentf-exclude` に `elpa/`・`.git/`・状態ファイル自身を登録)、② `savehist-mode`(ミニバッファ/M-x 履歴の永続化。将来の vertico 系刷新の土台にもなる)、③ `save-place-mode`(ファイル毎カーソル位置復元)、④ `which-key-mode`(**Emacs 30 で本体同梱**、追加 vendoring 不要。`which-key-idle-delay` 0.8)、⑤ `global-auto-revert-mode`(`global-auto-revert-non-file-buffers t` で dired 等も対象)、⑥ `repeat-mode`(同種コマンドの連打)。**保留/不採用(ユーザー判断)**: `delete-selection-mode`(保留 — `transient-mark-mode nil` 方針との整合を別途検討)、`hl-line-mode`(入れない)。状態ファイル `.recentf`/`.savehist`/`.saveplace` は `.gitignore` で除外(per-machine)
...
- [-] 縦横分割トグル `window-toggle-division`、`other-window-or-split`(C-tab) — **移植しない**(ユーザー判断)。**代替案**: ① **縦横トグル**は組み込みで一発の equivalent は無いが、`delete-other-windows` (`C-x 1`) → `split-window-{below,right}` (`C-x 2` / `C-x 3`)の組合せで再構築できる。自前 defun を書くなら現フレームのウィンドウ数を見て `delete-other-windows` + 逆方向 split のラッパ 10 行程度。② **`other-window-or-split`** は `(if (one-window-p) (split-window-sensibly) (other-window 1))` の 1 行 defun で代替可。③ ウィンドウ間移動だけなら **組み込み `windmove-{left,right,up,down}`**(Emacs 27+)を `S-<矢印>` 等に bind する `(windmove-default-keybindings)` で十分なケースも多い
