...
- [-] `C-t` / `C-z` の独自プレフィックスキーマップ(`C-t` 定義のみ済、配下バインドは未) — **移植しない**(ユーザー判断)。**現状**: `C-z` は既に **tab-bar 操作プレフィックス**として完全移植済み(elscreen 互換、`C-z c`/`k`/`n`/`p`/`1..9`/`r`/`b`/`f`/`'` 等を提供)。`C-t` は `init.el` で prefix キーマップとして定義のみ残しているが、旧の配下コマンド(`window-toggle-division`/`swap-screen`/`enlarge-window-auto`/`moccur-grep-find` 等)はいずれも本リビルドで `[-]` 不採用または別キー移植済(deadgrep / multiple-cursors の `C->` 等)のため**配下バインドは空のまま**。**代替案**: 上記 D 区分のいずれかを後日復活させる際に `C-t` 配下に bind を追加する余地として保持。それまでは空 prefix のままで実害なし(発火しない)
...
- [x] `vterm`(Emacs 内ターミナル) — **新規追加**(旧設定では未使用、ユーザー要望で導入)。判断記録: ① 当初候補 `eat` は issue #262(全角/罫線連続で O(n²) 再描画ハング、Claude Code バナーで再現)のため**不採用**、② `vterm`(libvterm ベース)を**採用**。ネイティブモジュール `vterm-module` を使うため各マシンで初回 `M-x vterm` 時にローカルビルド(要 cmake/libtool/C コンパイラ)、③ `elpa/` には `.el` ソースのみ vendoring し、`vterm-module.so` / `build/` は `/tree-sitter/` dylib と同様 `.gitignore` 除外(arch 依存ビルド成果物=同期しない)、④ 起動は **`M-x vterm` のみ**(グローバルキー未割当)、⑤ tmux 不使用のためキー例外なし、⑥ zsh 連携(`cd` 追従のディレクトリ追跡)は別リポ `~/.config/zsh`(chezmoi 不使用)管理のため**ここでは触らず README にスニペット提示のみ**、⑦ PATH は既設の `exec-path-from-shell` で GUI/daemon に取り込み済み(`claude` 等の alias は vterm が起動する対話 zsh が解決)。`module-file-suffix` が nil のマシンは `use-package :if` で全体スキップ(起動エラーなし)。使い方(コピーモード・ディレクトリ追跡スニペット・新マシン手順)は README.md に集約
